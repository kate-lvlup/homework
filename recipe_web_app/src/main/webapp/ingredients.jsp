...
<form>
    <div class="container">
        ...
        <c:forEach var="ingredientType" items="${ingredientMap}">
            <div class="product-block" style="background-image: url(${pageContext.request.contextPath}/img/${ingredientType.key.toLowerCase()}.png);">
                <div class="product-type">${ingredientType.key}</div>
                <ul class="product-list">
                    <c:forEach var="ingredient" items="${ingredientType.value}">
                        <li><input type="checkbox"><label>${ingredient.name}</label></li>
                    </c:forEach>
                </ul>
            </div>
        </c:forEach>
